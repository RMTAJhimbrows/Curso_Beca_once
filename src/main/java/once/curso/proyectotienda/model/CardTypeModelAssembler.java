...
public class CardTypeModelAssembler implements SimpleRepresentationModelAssembler<CardType> {
	...
	public void addLinks(EntityModel<CardType> cardType) {
		...
		cardType.add(WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CardTypeRestController.class)
				.findById(cardType.getContent().getId())).withSelfRel());
	}
	...
	public void addLinks(CollectionModel<EntityModel<CardType>> cardType) {
		...
		cardType.forEach(c ->{
			c.add(WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CardTypeRestController.class)
